...
util.sendWelcomeMessage = async function sendWelcomeMessage(channel, user, token) {
	if (channel.guild && !channel.guild.settings.get('welcome', true)) return;
	const minPostCount = channel.client.provider.get('global', 'minPostCount', config.get('minPostCount'));
	let minAge = channel.client.provider.get('global', 'minAge', config.get('minAge'));
	minAge = moment.duration(minAge, 'seconds').humanize();

	await channel.send(`Hello <@${user.id}>! To access more channels, you can link your Facepunch forums account. It must have ${minPostCount} posts or be ${minAge} old.
To do so you need to edit your Facepunch profile (https://facepunch.com/profile.php?do=editprofile) and set your **Flickr username** to **${escapeMarkdown(token)}**
	...
	return async () => {
		...
		for (const guild of discordClient.guilds.values()) {
			...
			for (const member of guild.members.values()) {
				...
				if (banRole && member.roles.has(banRole)) {
					...
					if (bans.length === 0) {
						try {
							...
							if (memberRole && !member.roles.has(memberRole) && user.facepunchId) await member.addRole(memberRole);
						} catch (e) {
							if (e.name === 'DiscordAPIError') {
								logger.warn(`Could not update ${member.user.username}#${member.user.discriminator} (${member.id})'s roles, ${e.message}.`);
							} else {
				...
				let joinedTimestamp = member.joinedTimestamp / 1000;
				const latestMessage = util.lastUserMessage(member, entryRoom);
				if (latestMessage) joinedTimestamp = latestMessage.createdAt.getTime() / 1000;
...
util.lastUserMessage = function lastUserMessage(user, channel) {
	...
	for (const message of channel.messages.values()) {
		...
		if (message.createdAt > maxDate) {
			maxDate = message.createdAt;
...
util.log = function log(guild, message, level = 'info') {
	...
	if (logChannel) return logChannel.send(message);
	if (moderatorChannel) return moderatorChannel.send(message);
