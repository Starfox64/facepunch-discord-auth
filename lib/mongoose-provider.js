 ...
 */
class MongooseProvider extends SettingProvider {
	/**
	 * @param {Model} model - Model for the provider
	 * @param {String} [idKey='id'] - Name of the field where the guild id will be stored
	 * @param {String} [dataKey='data'] - Name of the field where the guild data will be stored
	 */
	constructor(model, idKey = 'id', dataKey = 'data') {
		super();

		/**
		 * Model that will be used for storing/retrieving settings
		 * @type {Model}
		 */
		this.model = model;

		/**
		 * Name of the field where the guild id will be stored
		 * @type {String}
		 */
		this.idKey = idKey;

		/**
		 * Name of the field where the guild data will be stored
		 * @type {String}
		 */
	...
	async init(client) {
		...
		for(const row of rows) {
			const guild = row[this.idKey];
			const settings = row[this.dataKey];
	...
	async set(guild, key, val) {
		...
		const query = {};
		query[this.idKey] = guild;

		const doc = {};
		doc[this.dataKey] = settings;
		await this.model.update(query, doc, { upsert: true });
